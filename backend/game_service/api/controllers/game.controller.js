...
export const gameController = {
  async createGame(request, reply) {
    try {
      ...
      request.log.error(`createGame: failed to create game: ${err.message}`);
      return handleError(err, reply, `Failed to create the game: ${err.message}`);
    }
  },
	async finishGame(request, reply) {
		try {
			...
			request.log.error(`finishGame: failed to update finished game ${request.params.id}: ${err.message}`);
