...
export const profileService = {
  ...
  async getStats(id) {
    ...
    if (!user) throw new ErrorNotFound(`getStats: User ${id} cannot be found`);
    return user;
  },
  ...
  async addFriend(id, friendUsername) {
    ...
    if (!friend) throw new ErrorNotFound(`User ${friendUsername} not found`);
    const user = await prisma.user.findUnique({
      where: { id: id },
      select: {
        id: true,
    ...
    if (!user) throw new ErrorNotFound(`User ${id} not found`);
    if (id === friend.id) {
  ...
  async deleteFriend(id, friendUsername) {
    ...
    if (deleted.count === 0) {
      throw new ErrorNotFound(
        `No friendship exists between user ${id} and user ${friend.id}`,
      );
    }
    return friend.id;
  },
